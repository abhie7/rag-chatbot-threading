...
def register():
    ...
    user = User(email=email, password=password, displayName=display_name)
    user.save()
...
def login():
    ...
    if user and user.comparePassword(password):
        # Generate a token
        token = jwt.encode({'user_id': user.id}, secret_key, algorithm='HS256')

        # Return the user data and token
        return jsonify({'token': token, 'user': {'email': user.email, 'displayName': user.displayName}}), 200
    else:
